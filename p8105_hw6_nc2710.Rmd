...
The code chunk below creates a city_state variable (e.g. “Baltimore, MD”) and a binary variable indicating whether the homicide is solved. We also omit cities Dallas, TX; Phoenix, AZ; and Kansas City, MO – these don’t report victim race. We omit Tulsa, AL – this is a data entry mistake. The code modifies victim_race to have categories white and non-white, with white as the reference category. We also ensure sure that victim_age is numeric.
...
homicide_data = 
  ...
  mutate(victim_race = str_replace(victim_race, "Hispanic", "Non-White"),
         ...
         victim_race = fct_relevel(victim_race, "White", "Non-White", "Unknown"))
         # Above modifies the victim_race variable to have categories White, Non-White, and Unknown, with White as the reference category

         # What about Unknown race ???
...
glm_baltimore = 
  baltimore_df %>% 
  glm(is_solved ~ victim_age + victim_sex + victim_race,
      data = .,
      family = binomial()) %>% 
  broom::tidy() %>% 
  ...
  knitr::kable()
# hm, shouldn't there be two indicator variables for race? Should I filter to remove rows that had "Unknown" race before running the model? 
